...
test('test graph bfs', () => {
    ...
    // Self-Path
    var path1 = graph.path(c, c, algorithm='bfs')
    expect(path1[0].obj).toEqual("C")

    // Empty Path
    var path2 = graph.path(a, e, algorithm='bfs')
    expect(path2.length).toEqual(0)
    var path3 = graph.path(b, e, algorithm='bfs')
    expect(path3.length).toEqual(0)
    var path4 = graph.path(d, e, algorithm='bfs')
    expect(path4.length).toEqual(0)

    // Path
    var path5 = graph.path(a, b, algorithm='bfs')
    expect(path5[0].obj).toEqual('A')
    expect(path5[1].obj).toEqual('B')

    var path6 = graph.path(c, d, algorithm='bfs')
